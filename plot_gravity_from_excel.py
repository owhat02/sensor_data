# gravity (same-timestamp spreading by index)
# - 같은 시간에 여러 샘플이 있을 때, 인덱스 순서대로 T..T_next 구간에 균등 분할해 배치
# - x/y/z(그리고 |g| 옵션) 모두 같은 x축(t_plot) 사용
...
def build_plot_time_by_timestamp(df: pd.DataFrame, time_col: str,
    ...
    - fallback='jitter': 그런 경우 고정 지터(기본 1ms)만큼 벌려 배치
...
def main():
    ...
    if SHOW_MAGNITUDE and mag is not None:
        ln_m, = ax.plot(t_plot, mag, marker="o", linestyle="-", markersize=3, label="|g|")
